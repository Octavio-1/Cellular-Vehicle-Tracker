...
const int SEND_DELAY = 5000; // The time to wait before sending the next coordinates
...
static void readATString(boolean watchTimeout = false) {
  ...
  while (1) {
    ...
    if (watchTimeout) {
      ...
      if (newTime - time > 300000) error(ERROR_GSM_FAIL);
    }
...
static void sendData(const char* data) {
  ...
  sendATCommand("AT+SDATACONF=1,\"TCP\",\"52.197.83.84\",81");
